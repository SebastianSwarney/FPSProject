...
public class FireBehaviour_Scatter : FireBehaviour_Base
{
    ...
    public override void FireBullet(PhotonView p_firedPhotonView, int p_bulletOwner, TeamLabel p_teamLabel, GameObject p_bullet, Transform p_fireSpot, float p_bulletSpeed, float p_bulletDamage, Vector2 p_bulletSpread, Transform p_target)
    {
        ...
        for (int i = 0; i < m_bulletAmount; i++)
        {
            ...
            string newBullet = SerializeBulletData(p_teamLabel.m_myTeam, p_bullet.name, p_fireSpot.position, newDir, p_bulletSpeed, p_bulletOwner, damagePerBullet, p_target);
